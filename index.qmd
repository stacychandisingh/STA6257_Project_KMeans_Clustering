...
In cluster 1 there are 1,286 records. 
In cluster 2 there are 2,177 records. 
In cluster 3 there are 3,102 records.
```{r}
...
BSS/TSS Ratio
The ratio of between-cluster sum of squares(BSS) to total sum of squares (TSS). This measurement shows how well spread the clusters are between a value of 0 and 1. The closer to 1, the more distinct the clusters are within the dataset. In our model, the BSS/TSS ratio is 0.2042409, which is a pretty low ratio for this type of model. However, we determine a low number of clusters was sufficient in this model, which would also result in a low BSS/TSS ratio. 
...
We would like to see the number of popular songs in each cluster. Just like our previous analysis we take our spotify review set and see the number of popular tracks in each cluster based on our popularity score of >57. In cluster 1 there were 414 popular tracks, cluster 2 454 popular tracks, and cluster 3 1329 popular tracks. The graph compares the groups as we use this new only_pop data to see what type of popular tracks are in each cluster.  

```{r}
spotify_final_pop <- spotify_final %>%
  mutate(popular = if_else(track_popularity >= 57, "1", "0"))

head(spotify_final_pop)
...
```{r}
spotify_review <- only_pop %>%select(track_name, track_popularity, acousticness, danceability, energy, liveness, speechiness, valence,cluster) %>%
...
In this study, our primary objective was to showcase the application of the K-Means clustering algorithm using Spotify music data. We embarked on a journey of data preparation, meticulously cleaning and scaling musical features, transforming them into a suitable format for an unsupervised K-Means clustering algorithm. Our aim was to uncover the optimal number of clusters generated by this algorithm, essentially grouping similar musical attributes together based on their proximity in Euclidean distance.
 
Our analysis led us to the identification of three distinct clusters, each representing a set of musical features that could be associated with different genres. These clusters effectively encapsulate various values of specific attributes, paving the way for an efficient method of categorizing musical tracks within the Spotify dataset. This categorization can be immensely beneficial in guiding listeners towards tracks that align with their musical preferences, enhancing their overall listening experience.
 
It's worth noting that the K-Means algorithm can be approached from various angles, and our study serves as a foundation for further exploration. We encourage future research to delve into this analysis by employing alternative K-Means clustering algorithms, to investigate whether the results align with our findings or offer new and diverse insights. Such endeavors could enrich our understanding of the nuances of music categorization and recommendation systems, contributing to a more nuanced and personalized music discovery process for Spotify users.

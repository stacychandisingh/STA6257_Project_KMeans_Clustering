...
Two step clustering for data reduction combining DBSCAN and k-means clustering by B.J.J. Kremers, A. Ho , J. Citrin, K.L. van der Plassche

The article I read was Two step clustering for data reduction combining DBSCAN and k-means clustering. The use of DBSCAN and k-means clustering algorithms in two – step process to see a density dependent reduction ratio. This article is important information because it allows the DBSCAN to help distinguish between different regions of data density. It shows the potential for data reduction without impacting the accuracy of the results. This algorithm was applied to a training neural network surrogate model of fusion plasma. The use of this algorithm in the dataset showed that it can be reduced by about 20, before there is a loss in accuracy of the neural network surrogate models. The resulting reduction helps analyze the data already there, which in end lowers the cost of repopulating the same area with the data. This can lead to expansion of the parameter space in the dataset. It can also help maintain the dataset being constant by reducing and detection as well. Some suggestions maybe to use the algorithm in enhancing the reduction factor itself to make it a bit more accurate when working with datasets.

Kremers, Citrin, J., Ho, A., & Plassche, K. L. (2023). Two‐step clustering for data reduction combining DBSCAN and k‐means clustering. Contributions to Plasma Physics (1988), 63(5-6). https://doi.org/10.1002/ctpp.202200177

**Summary 6:**

k-means clustering with outlier removal by Guojun Gana,∗ , Michael Kwok-Po Ng b

This week I read k-means clustering with outlier removal by Guojun Gana Michael Kwok-Po Ng. This article goes over the KMOR algorithm to perform data clustering and outlier detection. It focuses on parameters n0 and y to control the numbers of outliers. The importance of this article shows clustering and outlier detection, it also shows importance in improving efficiency as well. The tests that were conducted was on both synthetic and real datasets. The KMOR algorithm was effective in clustering data while it detected outliers. The KMOR in this study outperformed k-means, ODC, and NEO-k-means algorithm in detecting outliers whilst being very accurate. This was also used in large datasets which KMOR showed to be more effective. The results show that the kmor algorithm outperformed the others in accuracy and runtime. It highlights how applicable KMOR can be in other real world situations. Some limitations that could of been explored are alternative outlier control and detection. 

Gan, & Ng, M. K.-P. (2017). k-means clustering with outlier removal. Pattern Recognition Letters, 90, 8–14. https://doi.org/10.1016/j.patrec.2017.03.008
...
An efficient k′-means clustering algorithm by Krista Rizman Žalik

This article I read was An efficient k0 -means clustering algorithm by Krista Rizman Zˇalik. This article goes over k-means algorithm without the necessity to preassign the number of clusters. So in other words it is made for clustering data without the prior knowledge of clusters.This is achieved by minimizing the novel cost function which is used in k-means algorithm. There is a process where initial clustering and adjustment of seed points to minimize the cost function. This entails to find the correct number of clusters and locate them as well. The importance of this article is improved cost functions and the efficiency of the clustering in datasets. The results showed that it was efficient in the actual number of clusters in datasets. The results also show that it performs in high dimensional spaces, it randomly selects initial cluster centers from the dataset to avoid issues. The paper does show that there are a large range of k for each datset, The range of the parameter k in the algorithm. It is not much larger than k but when it is greater is when there is that large range. A limitation with this is if the clusters do overlap, the algorithm can regard it a cluster which leads to incorrect results.

Žalik. (2008). An efficient k′-means clustering algorithm. Pattern Recognition Letters, 29(9), 1385–1391. https://doi.org/10.1016/j.patrec.2008.02.014

**Summary 6:**

Performance Appraisal and Automatic Scoring System for College Counselors Based on Kmeans Clustering by  Zhichao Wang and Qing Tian 

The other article I read is Performance Appraisal and Automatic Scoring System for College Counselors Based on K-means Clustering by Zhichao Wang ,and Qing Tian.  This article is to establish a performance appraisal scoring system for college counselors using K-means Clustering. This is important to improve and figure out university goals, department goals that are trying to be attained by the university. This can help with student development, improvement, retaining of students as well. The model that was used was the GAPSO model which breaks down to genetic algorithm and particle swarm optimization. The steps that were used were initialization of the GA population, set the parameters for both algorithms. The next step was to calculate individual and population extreme within the PSO populations, then to select the solution with the GA through group optimization. Next, the solution is to be compared by both populations. Then, transfer the solution to see which population has the better solution. Then to make the adjustments and then crossover mutation operations, which then go through another round of iteration to find the solution within the spaces. The counselor goals are given through an expert outside group. The goals can range from career development, student development, student services etc. The goal is for this appraisal plan to guide the counselors in their work. They have started pilot of the appraisal system. The improvement can be how the system can improve the work of the counselors outside of those certain levels.


Wang, & Tian, Q. (2022). Performance Appraisal and Automatic Scoring System for College Counselors Based on Kmeans Clustering. Mathematical Problems in Engineering, 2022, 1–12. https://doi.org/10.1155/2022/4167842
...
Data‐Driven Velocity Model Evaluation Using K‐Means Clustering by Xiong, Neng Qiu, Hongrui Niu, Fenglin

A peer reviewed article I read is Data‐Driven Velocity Model Evaluation Using K‐Means Clustering by Xiong, Neng Qiu, Hongrui Niu, Fenglin. This article goes in about evaluating velocity models using K-means clustering method to observed surface wave phase velocity dispersion maps. These models that were used CVM-H15.1 and CVM-S4.26, the idea was to compare clustering patterns synthetic and observed data to see the quality of the velocity models. This research is valuable to help with geological and geophysical area of study. It can help in understanding earthquake research. It also is important to understand seismic data when it comes to hazard assessment in those events. The way this is solved is by calculating the phase velocity of CVM-H15.1 and CVM-S4.26, which then the k-means clustering is applied to it. The Jaccard similarity coefficient is then used to see if the difference between the two. The results showed that 57.4 percent of the CVM-S4.26 averaged through the clusters while the CVM-H15.1 had 18.6 percent. Some suggestions for the study can be said is to use other types of seismic data to assess and compare other velocity models.

Xiong, N., Qiu, H., & Niu, F. (2021). Data-driven velocity model evaluation using K-means clustering. Geophysical Research Letters, 48, e2021GL096040. https://doi-org.ezproxy.lib.uwf.edu/10.1029/2021GL096040

**Summary 6:**

Skin cancer detection from dermoscopic images using deep learning and fuzzy k-means clustering by Nawaz, M., Mehmood, Z., Nazir, T., Naqvi, R. A., Rehman, A., Iqbal, M., & Saba, T.

One that was interesting to me was Skin cancer detection from dermoscopic images using deep learning and fuzzy k-means clustering in this article goes over melanoma skin cancer detection. The challenge is distinguishing between healthy skin moles, and which are cancerous. So, what the researchers used was a deep learning approach region based neural networks and fuzzy k-means clustering to detect it on its earliest stages. The methods used specifically the faster region-based convolutional neural networks known as (RCNN) as well as fuzzy k-means clustering. This method has a few steps to detect melanoma, and the challenges it comes with. The process starts with preprocessing which works with noise and illumination issues, and visual information. The next step is artifact removal which is eliminating hair or blood vessels. They use of enhanced image quality which then RCNN is used to extract features from the images. Next, the fuzzy k-means clustering is used to segment the melanoma areas. The result of the method was it outperformed state of the art methods, by the results shown by the three datasets as goes  95.4 %,(ISBI-2016) 93.1% (ISIC-2017,) 95.6%(PH2) of early detection rate for melanoma. The importance of these results show that the RCNN does well in its ability to generate efficient feature from input images. While the FKM clustering does well in segmenting the melanoma itself depending on shape and size of the areas. These two can help detect early melanoma which can save countless lives due to its accuracy and efficiency.
...
16.	Zubair, M., Iqbal, M.A., Shil, A. et al. An Improved K-means Clustering Algorithm Towards an Efficient Data-Driven Modeling. Ann. Data. Sci. (2022). https://doi.org/10.1007/s40745-022-00428-2
